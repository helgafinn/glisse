...
│   └── AppIcon.icns              generated from LogoArtwork.swift
├── Scripts/
│   ├── build.sh                  filters SwiftPM's very verbose failures
│   ├── make-icon.sh              wraps `Glisse --export-icon` + iconutil
│   └── make-signing-identity.sh  optional stable local signing identity
├── docs/
│   └── QA-CHECKLIST.md           manual hardware test sheet
│
├── Sources/GlissePrivate/        ALL unsafe / private code lives here
│   ├── include/
│   │   ├── GLMultitouchTypes.h   stable GLRawTouch, layout descriptor
│   │   ├── GLMultitouchBridge.h  raw trackpad contacts
│   │   ├── GLHapticActuator.h    MTActuator; works from a background app
│   │   ├── GLBrightnessBridge.h  DisplayServices + CoreDisplay + IODisplay
│   │   ├── GLOSDBridge.h         system HUD: OSDUIHelper + OSDManager routes
│   │   └── GLDDCBridge.h         IOAVService / IOI2C transport
│   ├── GLMultitouchBridge.m      dlsym, layout validation, frame conversion
│   ├── GLHapticActuator.m
...
├── Sources/GlisseKit/
│   ├── App/
│   │   ├── GlisseMain.swift           entry + terminal diagnostic modes
│   │   ├── AppDelegate.swift
│   │   ├── AppCoordinator.swift       owns every service; startup order
...
│   │   ├── AxisOrientationVerifier.swift  confirms +y is up, from real data
│   │   ├── Private/
...
│   │   └── HUDCoordinator.swift       OSD-direct vs media-keys vs none
│   ├── Feedback/HapticFeedbackService.swift
│   ├── Input/
│   │   ├── KeyboardMonitor.swift
│   │   ├── ModifierMonitor.swift
│   │   ├── MediaKeyController.swift   synthesises the real media keys
...
│   └── Utilities/
│       ├── Branding.swift             the display name, in one place
│       ├── LogoArtwork.swift          mark + wordmark, drawn in code
│       ├── Clamp.swift
│       ├── Throttler.swift            Throttler, Coalescer, MonotonicClock
│       └── Logger.swift               OSLog categories
│
├── Sources/Glisse/main.swift
└── Tests/GlisseTests/                 149 tests
    ├── GestureTestSupport.swift
    ├── EdgeGestureEngineTests.swift   the gesture matrix
    ├── ThreeFingerTapTests.swift
    ├── GestureSessionTests.swift      + typing suppression, throttler
    ├── MappingTests.swift             volume, mute, DDC, HUD, haptics, keys
